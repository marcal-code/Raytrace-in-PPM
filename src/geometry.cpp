...
float Sphere::intersects(const Ray &ray) {
    ...
    // float a = util::Vec3::dot(ray.direction, ray.direction);
    ...
    util::Vec3 oc = ray.origin - center;

    float a = 1.0f;
    float b = 2.0f * util::Vec3::dot(oc, ray.direction);
    float c = util::Vec3::dot(oc, oc) - radius * radius;
